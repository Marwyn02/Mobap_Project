...
export default function Result1() {
  ...
  // Retry the quiz 1 again
  const handleRetry = () => {
    router.replace("/quiz1");
  };

  // Proceed to the quiz 2
  const handleNextQuiz = () => {
    router.replace("/quiz2");
  };
  return (
    <View style={styles.container}>
      <Text style={styles.resultTitle}>Quiz 1 Result</Text>
      ...
      {percentage < 80 ? (
        <Text style={styles.retryMessage}>
          ...
          required <Text style={styles.requiredScore}>8 score</Text>. Please
          retry the quiz to improve your understanding and try again.
        </Text>
      ) : (
        <View>
          <Text style={styles.passMessage}>You've passed the Quiz 1!</Text>
          <Text style={styles.proceedMessage}>
            Click the proceed to continue to Level 2.
          </Text>
        </View>
      )}
